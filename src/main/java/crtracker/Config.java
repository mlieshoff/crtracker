...
public class Config {
  ...
  public DiscordApi createBot() throws LoginException, InterruptedException, RateLimitedException {
    return new DiscordApi(config, credentials);
  }

  public ApiWrapper createApiWrapper() {
    return new ApiWrapper(config.getProperty("proxy.cr.api.url"), credentials.getProperty("proxy.cr.api.token"),
        config.getProperty("official.cr.api.url"), credentials.getProperty("official.cr.api.token"));
  }
